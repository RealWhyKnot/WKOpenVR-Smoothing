...
1. Close `OpenVR-WKSmoothing.exe` and SteamVR.
...
3. If OpenVR-WKSpaceCalibrator is also installed, leave the rest of `01openvrpair/` alone. SC owns the matching `enable_calibration.flag` and keeps calibration working.
4. If you don't have SC installed and want the shared driver gone too, delete the entire `01openvrpair/` folder.
5. Optionally delete `%LocalAppDataLow%\OpenVR-WKSmoothing\` to remove the saved settings.
